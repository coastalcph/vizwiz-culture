...
class COCO:
    def __init__(self, dataset=None):
        ...
        if dataset is not None:
            if type(dataset) == str and os.path.isfile(dataset):
    ...
    def showAnns(self, anns):
        ...
        if self.dataset["type"] == "instances":
            ...
            p = PatchCollection(
                polygons, facecolors=color, edgecolors=(0, 0, 0, 1), linewidths=3, alpha=0.4
            )
            ax.add_collection(p)
    ...
    def loadRes(self, anns):
        ...
        if type(anns) == str and os.path.isfile(anns):
